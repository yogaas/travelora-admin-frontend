...
import { PaymentMethod, paymentMethodService, PaymentMethodInput } from "../../services/paymentMethod.service";
...
export function PaymentMethodFormModal({ isOpen, onClose, methodToEdit }: PaymentMethodFormModalProps) {
  ...
  }, [methodToEdit, isOpen, reset]);

  const createMutation = useMutation({
    mutationFn: (data: PaymentMethodInput) => paymentMethodService.createPaymentMethod(data),
    onSuccess: (res) => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: Partial<PaymentMethodInput>) => paymentMethodService.updatePaymentMethod(methodToEdit!.id, data),
    onSuccess: (res) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm transition-opacity">
      <div className="bg-white rounded-[24px] shadow-premium w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col">
        ...
        <div className="p-6">
          <form id="payment-method-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
